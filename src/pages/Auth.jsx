        ...
        <section className="bg-[#fbf9f0] p-6 sm:p-8 md:p-10 border-2 border-[#5f5e5e] shadow-[10px_10px_0px_#006d41] relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
            {(!isLogin && !isForgot) && (
              <div className="relative group">
                <label className="block font-label text-[10px] font-bold text-primary/70 mb-2 uppercase tracking-widest">
                  [00] OPERATOR_ID
                </label>
                <div className="flex items-center border-b-2 border-[#5f5e5e]/30 group-focus-within:border-secondary transition-all pb-1">
                  <span className="text-secondary font-bold mr-2 text-xl tracking-tighter">&gt;</span>
                  <input required={!isLogin} disabled={isLoading} value={operator} onChange={(e) => setOperator(e.target.value)} className="w-full bg-transparent border-none focus:ring-0 outline-none text-primary font-bold placeholder:text-primary/30 placeholder:font-normal uppercase py-1 disabled:opacity-50" placeholder="NEW_OPERATOR" type="text" />
            ...
            <div className="relative group">
              ...
              <div className="flex items-center border-b-2 border-[#5f5e5e]/30 group-focus-within:border-secondary transition-all pb-1">
                <span className="text-secondary font-bold mr-2 text-xl tracking-tighter">&gt;</span>
                <input required disabled={isLoading} value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-transparent border-none focus:ring-0 outline-none text-primary font-bold placeholder:text-primary/30 placeholder:font-normal uppercase py-1 disabled:opacity-50" placeholder="OPERATOR@NETWORK.COM" type="email" />
                <span className="w-2.5 h-5 bg-secondary/40 hidden group-focus-within:block blink"></span>
              </div>
            </div>
            ...
            {!isForgot && (
              <div className="relative group">
                <label className="block font-label text-[10px] font-bold text-primary/70 mb-2 uppercase tracking-widest flex justify-between">
                ...
                <div className="flex items-center border-b-2 border-[#5f5e5e]/30 group-focus-within:border-secondary transition-all pb-1">
                  <span className="text-secondary font-bold mr-2 text-xl tracking-tighter">&gt;</span>
                  <input required disabled={isLoading} value={passphrase} onChange={(e) => setPassphrase(e.target.value)} className="w-full bg-transparent border-none focus:ring-0 outline-none text-primary font-bold placeholder:text-primary/30 py-1 disabled:opacity-50" placeholder="••••••••" type="password" />
                  <span className="w-2.5 h-5 bg-secondary/40 hidden group-focus-within:block blink"></span>
                </div>
              </div>
            )}
          ...
          <div className="mt-8 pt-4">
            <div className="flex items-center gap-4 mb-5">
              ...
              <span className="font-label text-[10px] font-bold text-primary/50 uppercase tracking-widest">Or Continue With</span>
              <div className="flex-grow border-t-2 border-[#5f5e5e]/15"></div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button type="button" disabled={isLoading} onClick={() => !isLoading && googleLogin()} className="btn-3d-secondary w-full bg-[#fbf9f0] text-primary py-3 px-4 font-bold flex items-center justify-center gap-3 disabled:opacity-50">
              ...
              <button type="button" disabled={isLoading} onClick={() => !isLoading && window.location.assign(`https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&scope=user:email`)} className="btn-3d-secondary w-full bg-[#fbf9f0] text-primary py-3 px-4 font-bold flex items-center justify-center gap-3 disabled:opacity-50">
